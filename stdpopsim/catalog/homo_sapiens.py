...
class _KammAncientEurasia(HomoSapiensModel):
    ...
    name = "Complex multi-population model of ancient Eurasia (Kamm et al. 2019)"
    description = """
        This is the best-fitting model of a history of multiple ancient and present-day human populations 
        sampled across Eurasia over the past 120,000 years. The fitting was performed using momi2 (Kamm et al. 2019), 
        which uses the multi-population site-frequency spectrum as input data. The model includes a ghost admixture event
        (from unsampled basal Eurasians into early European farmers), and two admixture events where the source is 
        approximately well-known (from Neanderthals into Non-Africans and from Western European hunter-gatherers into modern
        Sardinians. There are three present-day populations: Sardinians, Han Chinese and African Mbuti. Additionally, 
        there are several ancient samples obtained from fossils dated at different times in the past: the Altai Neanderthal
        (Prufer et al. 2014), a Mesolithic hunter-gatherer (Lazaridis et al. 2014), a Neolithic early European sample 
        (Lazaridis et al. 2014), and two Palaeolithic modern humans from Siberia - MA1 (Raghavan et al. 2014) and  Ust'Ishim
        (Fu et al. 2014). All the ancient samples are represented by a single diploid genome.
    """
    populations = [
        stdpopsim.Population(name="Mbuti", description="Present-day African Mbuti"),
        stdpopsim.Population(name="BasalEurasian", description="Putative basal Eurasians", allow_samples=False),
        stdpopsim.Population(name="LBK", description="Representative of Linear Pottery culture - early European farmer (EEF) from the Neolithic"),
        stdpopsim.Population(name="Sardinian", description="Present-day Sardinian"),
        stdpopsim.Population(name="Loschbour", description="Representative of Mesolithic European Western hunter-gatherers (WHG)"),
        stdpopsim.Population(name="MA1", description="MAl'ta culture (Upper Palaeolithic northern Eurasian)"),
        stdpopsim.Population(name="UstIshim", description="Ust'-Ishim man, early Paleolithic hunter-gatherer from western Siberia"),
        stdpopsim.Population(name="Neanderthal", description="Altai Neanderthal from Siberia")
    ]
    citations = [
        stdpopsim.Citation(
            author="Kamm et al.",
    ...
    def __init__(self):
        ...
        N_MA1 = N_Losch
        N_Ust = N_Losch
        t_NeaPopSizeChange = 200000 / self.generation_time
        # Note that the original Kamm et al. model has the Altai Neanderthal as sampled *after* the Nea->NonAfr admixture event
        # Due to re-dating of the Altai Neanderthal sample, we now know that the sampling happened much earlier
        # The new date is the one specified in this particular implementation of the model
        ...
        # Given that we're using best model estimate, ghost WHG is directly descended from Loschbour, so this parameter is not used
        ...
        # Sampling times (obtained from literature)
        samp_Mbuti = 0
        samp_LBK = 7000 / self.generation_time
        samp_Sard = 0
        samp_Losch = 8000 / self.generation_time
        samp_MA1 = 24000 / self.generation_time
        samp_Han = 0
        samp_Ust = 45000 / self.generation_time
        samp_Nea = 120000/ self.generation_time
        ...
        # Population IDs: Mbuti = 0; Basal Eurasian = 1; LBK = 2; Sardinian = 3; Loschbour = 4
        ...
        self.demographic_events = [
            ...
            msprime.MassMigration(
                time=t_GhostWHG_to_Sard, source=3, destination=4, proportion=p_GhostWHG_to_Sard),
            # Sardinian merges into LBK / EEF
            msprime.MassMigration(
                time=t_Sard_LBK, source=3, destination=2, proportion=1.0),
            # LBK / EEF receives admixture from Basal Eurasians
            msprime.MassMigration(
                time=t_Basal_to_EEF, source=2, destination=1, proportion=p_Basal_to_EEF),
            # LBK / EEF merges into Loschbour
            msprime.MassMigration(
                time=t_LBK_Losch, source=2, destination=4, proportion=1.0),
            # MA1 merges into Loschbour
            msprime.MassMigration(
                time=t_MA1_Losch, source=5, destination=4, proportion=1.0),
            # Han merges into Loschbour
            msprime.MassMigration(
                time=t_Han_Losch, source=6, destination=4, proportion=1.0),
            # UstIshim merges into Loschbour
            msprime.MassMigration(
                time=t_Ust_Losch, source=7, destination=4, proportion=1.0),
            # Loschbour / Non-Africans receive admixture from Neanderthals
            msprime.MassMigration(
                time=t_Nean_to_Eurasian, source=4, destination=8, proportion=p_Nean_to_Eurasian),
            # Basal Eurasians merge into Loschbour / Non-Africans
            msprime.MassMigration(
                time=t_Basal_Losch, source=1, destination=4, proportion=1.0),
            # Mbuti merge into Loschbour / Non-Africans
            msprime.MassMigration(
                time=t_Mbu_Losch, source=0, destination=4, proportion=1.0),
            # Neanderthal change population size
            msprime.PopulationParametersChange(
                time=t_NeaPopSizeChange, initial_size=N_Loschbour, population_id=8),          
            # Neanderthal merge into Loschbour / modern humans
            msprime.MassMigration(
                time=t_Nean_Losch, source=8, destination=4, proportion=1.0)            
        ]
