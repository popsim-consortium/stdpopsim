...
class TestGenomicElementTypes(PiecewiseConstantSizeMixin):
    ...
    def verify_mutation_rates(self, contig, ts):
        ...
        empty_intervals = np.array([[0, int(contig.length)]])
    ...
    def verify_genomic_elements(self, contig, ts):
        ...
        # getting the ids for the neutral mutation types
        neutral_ids = [
            mtdict["id"]
            for mtdict in contig.mutation_types()
            if mtdict["mutation_type"].is_neutral
        ]
        ...
        for j, (dfe, intervals) in enumerate(
            ...
            # figuring out if all mutation types are neutral in this GET
            all_neutral = all(
